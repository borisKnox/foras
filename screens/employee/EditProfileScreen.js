...
import {UserEditProfileData, WorkAreas, Languages, Skills} from '../../constants/Constants';
...
export default class EditProfileScreen extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            availableWorkFromTime: global.loginInfo.available_work_from_time == null? "" : global.loginInfo.available_work_from_time,
            experience: global.loginInfo.experience == null? "" : global.loginInfo.experience,
            language: "", 
            languages: global.loginInfo.languages == null? "" : global.loginInfo.languages, 
            selectedLanguage: [],
            skill: "",            
            skills: global.loginInfo.skills == null? "" : global.loginInfo.skills,            
            selectedSkills: [],
    ...
    componentDidMount(){
        if(this.state.languages != ""){            
            this.state.languages.split(",").map((data, index)=>{
                if(index + 1 < this.state.languages.split(",").length){
                    // lan[index] = {item: data}
                    this.state.selectedLanguage.push({item: data});
                }
            })
            this.forceUpdate()            
        }
        ...
        if(this.state.skills != ""){
            ...
            this.state.skills.split(",").map((data, index)=>{
                if(index + 1 < this.state.skills.split(",").length){
                    // lan[index] = {item: data}
                    this.state.selectedSkills.push({item: data});
                }
            })
            this.forceUpdate()            
        }
    ...
    saveProfile(){       
        ...
        this.state.selectedLanguage.map((data, index)=>{
            languages_select += data.item + ", "
        })

        var skills_select = '';
        this.state.selectedSkills.map((data, index)=>{
            skills_select += data.item + ", "
        })

        console.log("===========languages_select========", languages_select)
        this.setState({spinner: true});

        UserEditProfileData.logo = this.state.logo
        UserEditProfileData.name = this.state.name
        UserEditProfileData.work_area = this.state.workArea
        UserEditProfileData.city = this.state.city			
        UserEditProfileData.hourly_rate = this.state.hourlyRate				
        UserEditProfileData.available_work_from_time = this.state.availableWorkFromTime
        UserEditProfileData.experience = this.state.experience
        UserEditProfileData.languages = languages_select
        UserEditProfileData.skills = skills_select
        UserEditProfileData.about_me = this.state.aboutYou
        UserEditProfileData.token = global.token
        UserEditProfileData.role = "individual"
        // console.log("===UserEditProfileData===", UserEditProfileData)    


        api.userEditProfile(UserEditProfileData).then((res)=>{
            ...
            if(res.status == 200){
                ...
                api.uploadAvatar(global.token, this.state.logo).then((res)=>{
                    ...
                    if(res.status == 200){
                        ...
                        this.props.navigation.replace('MyProfile1');
                    }else{
                        Alert.alert(
                            '',
                            "error",
                ...
                Alert.alert(
                    ...
                    [typeof(res.errors.hourly_rate) == "undefined" ? "": res.errors.hourly_rate.toString()] + " " +
                    [typeof(res.errors.available_work_from_time) == "undefined" ? "": res.errors.available_work_from_time.toString()] + " " +
                    [typeof(res.errors.experience) == "undefined" ? "": res.errors.experience.toString()] + " " +
                    [typeof(res.errors.languages) == "undefined" ? "": res.errors.languages.toString()] + " " +
                    [typeof(res.errors.skills) == "undefined" ? "": res.errors.skills.toString()] + " " +                    
                            ...
                            <InputScrollView contentContainerStyle={{flex: 1}} topOffset={5}>
                                <View style={{width: Dimensions.get('window').width * 0.85, overflow: 'hidden' , borderRadius: 10, backgroundColor: 'white', alignItems: 'center', marginTop: 5}}>
                                    ...
                                    <View style={styles.itemBlock}>
                                        ...
                                        <View style={{marginTop: 10, marginBottom: 8}}>
                                            <View style={styles.formControl}>                                      
                                                <TextInput style={styles.formInput} keyboardType="numeric" placeholder={Labels._job_title} value={this.state.hourlyRate} onChangeText={text => this.setState({hourlyRate: text})}/>
                                            </View>
                                        </View> 
                                    </View>
